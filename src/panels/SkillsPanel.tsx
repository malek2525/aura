...
const SkillsPanel: React.FC<SkillsPanelProps> = ({ profile }) => {
  ...
  const [replies, setReplies] = useState<{ safe: string; direct: string; playful: string } | null>(null);
  ...
  const handleAskAura = async () => {
    ...
    try {
      const result = await generateReplyOptions(profile, incomingMessage.trim(), context);
      setReplies(result);
    } catch (err) {
    ...
    <div className="h-full flex flex-col p-4 lg:p-5">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-400">
          Paste a message you received, and Aura will draft three reply options matching your vibe.
        </p>
      </div>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-500">/ {card.sublabel}</span>
                  </div>
                  <button
        ...
        {!replies && !isLoading && !error && (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
            ...
            <p className="text-sm text-slate-400 max-w-xs">
              Paste a message above and click "Ask Aura" to get three tailored reply options.
            </p>
          </div>
        )}
      </div>
