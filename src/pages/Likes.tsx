...
export const Likes: React.FC<LikesProps> = ({ onUpgrade, onViewMatch }) => {
  return (
    <div className="h-full bg-white flex flex-col pt-4 pb-24 px-4">
      ...
      <div className="bg-text-main rounded-2xl p-5 mb-6 relative overflow-hidden">
        <div className="relative z-10">
          ...
          <button 
            onClick={onUpgrade}
            className="bg-gold text-text-main px-4 py-2 rounded-full text-xs font-bold hover:bg-white transition-colors"
          >
            Try Free for 3 days
          </button>
        </div>
        <div className="absolute top-0 right-0 w-32 h-32 bg-coral/20 blur-3xl rounded-full" />
      </div>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
          {[1, 2].map((i) => (
            <div 
              ...
              onClick={() => onViewMatch?.(i.toString())}
            >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div 
              key={i} 
              className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-warm-white border border-warm-gray"
            >
              <img 
              ...
              <div className="absolute bottom-3 left-3 right-3">
                <div className="h-2 w-16 bg-text-sec/10 rounded-full mb-1" />
                <div className="h-2 w-8 bg-text-sec/10 rounded-full" />
              </div>
            </div>
          ))}
        </div>
